...
class hybrid_stochastic_recommender:
...
def recommend_users(ds_name, dataset_ts, dataset_vs, set, thetas=[0.5], subm=False):
	...
	recommender = hybrid_stochastic_recommender([recommender1, recommender2], thetas=thetas)
	
	evaluators = {}
	for theta in thetas:
		filename='MAP_hybrid_stochastic_set=' + set + '_theta=' + "%.2f"%(theta) + '.txt'
		print filename
...
if __name__ == '__main__':
	...
	recommend_users(ds_name, dataset_ts, dataset_vs, set, thetas=[float(x)/100 for x in range(0, 110, 10)])
